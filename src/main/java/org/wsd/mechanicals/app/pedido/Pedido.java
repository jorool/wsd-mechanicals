...
public class Pedido implements Serializable {
	...
	public Pedido addProduto(Produto produto, Quantidade quantidade) {
		...
		if (!produtos.containsKey(produto)) {
			...
			produtos.put(produto, Ops.adicao(quantidadeAtual).somando(quantidade).somar());
		}
		return this;
	}
	...
	public Map<Produto, Quantidade> getProdutosNaoCancelados() {
		...
		for (Cancelamento cancelamento : cancelamentos) {
			...
			quantidade = Ops.subtracao(quantidade).subtraendo(cancelamento.getQuantidade()).subtrair();
	...
	public Quantidade getComissao() {
		...
		for (Produto produto : produtosNaoCancelados.keySet()) {
			...
			comissao = Ops.adicao(comissao).somando(comissaoProduto).somar();
		}
		return comissao;
	}
